defmodule BedTrackingGraphql.Schema.Report do
  ...
  object :report do
    ...
    field :total_non_available_beds_where_ward_type_covid_and_source_of_admission_ed, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"covid", "ed"})
      end)
    end

    field :total_non_available_beds_where_ward_type_green_and_source_of_admission_ed, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"green", "ed"})
      end)
    end

    field :total_non_available_beds_where_ward_type_amber_and_source_of_admission_ed, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"amber", "ed"})
      end)
    end

    field :total_non_available_beds_where_ward_type_covid_and_source_of_admission_internal_ward, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"covid", "internal_ward"})
      end)
    end

    field :total_non_available_beds_where_ward_type_green_and_source_of_admission_internal_ward, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"green", "internal_ward"})
      end)
    end

    field :total_non_available_beds_where_ward_type_amber_and_source_of_admission_internal_ward, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"amber", "internal_ward"})
      end)
    end

    field :total_non_available_beds_where_ward_type_covid_and_source_of_admission_internal_itu, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"covid", "internal_itu"})
      end)
    end

    field :total_non_available_beds_where_ward_type_green_and_source_of_admission_internal_itu, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"green", "internal_itu"})
      end)
    end

    field :total_non_available_beds_where_ward_type_amber_and_source_of_admission_internal_itu, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"amber", "internal_itu"})
      end)
    end

    field :total_non_available_beds_where_ward_type_covid_and_source_of_admission_external_ward, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"covid", "external_ward"})
      end)
    end

    field :total_non_available_beds_where_ward_type_green_and_source_of_admission_external_ward, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"green", "external_ward"})
      end)
    end

    field :total_non_available_beds_where_ward_type_amber_and_source_of_admission_external_ward, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"amber", "external_ward"})
      end)
    end

    field :total_non_available_beds_where_ward_type_covid_and_source_of_admission_external_itu, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"covid", "external_itu"})
      end)
    end

    field :total_non_available_beds_where_ward_type_green_and_source_of_admission_external_itu, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"green", "external_itu"})
      end)
    end

    field :total_non_available_beds_where_ward_type_amber_and_source_of_admission_external_itu, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"amber", "external_itu"})
      end)
    end

    field :total_non_available_beds_where_ward_type_covid_and_source_of_admission_itu_readmission, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"covid", "itu_readmission"})
      end)
    end

    field :total_non_available_beds_where_ward_type_green_and_source_of_admission_itu_readmission, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"green", "itu_readmission"})
      end)
    end

    field :total_non_available_beds_where_ward_type_amber_and_source_of_admission_itu_readmission, :integer do
      resolve(fn report, params, info ->
        Resolver.Report.dataloader_total_non_available_beds_where_ward_type_and_source_of_admission_where_date_admitted_yesterday(report, params, info, {"amber", "itu_readmission"})
      end)
    end
  end
