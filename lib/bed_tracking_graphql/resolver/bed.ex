defmodule BedTrackingGraphql.Resolver.Bed do
  ...
  def register_multiple(
    ...
    with {:ok, current_hospital_manager} <- Context.Authentication.current_hospital_manager(info),
         {:ok, beds} <-
           Context.Bed.register_multiple(number_of_beds, ward_id, params, current_hospital_manager) do
      {:ok, %{beds: beds}}
    end
  end
  ...
  def register(%{input: %{ward_id: ward_id}}, info) do
    with {:ok, current_hospital_manager} <- Context.Authentication.current_hospital_manager(info),
         {:ok, bed} <- Context.Bed.register(ward_id, current_hospital_manager) do
      {:ok, %{bed: bed}}
    end
  end
  ...
  def update(%{input: %{id: id} = params}, info) do
    with {:ok, current_hospital_manager} <- Context.Authentication.current_hospital_manager(info),
         {:ok, bed} <- Context.Bed.update(id, params, current_hospital_manager) do
      {:ok, %{bed: bed}}
    end
  end
  ...
  def discharge_patient(
    ...
    with {:ok, current_hospital_manager} <- Context.Authentication.current_hospital_manager(info),
         ...
         {:ok, success} <- Context.Bed.discharge_patient(id, params, current_hospital_manager) do
      {:ok, %{success: success}}
    end
  end
